    ...
    with path.open(encoding="utf-8", newline="") as fp:
        ...
        for row in reader:
            ...
            window = MintDumpFeatureWindow(
                ...
                holder_single_max_pct=parse_optional_decimal(row.get("holder_single_max_pct", "")),
                holder_entropy=parse_optional_decimal(row.get("holder_entropy", "")),
                holder_snapshot_lag_sec=parse_optional_float(row.get("holder_snapshot_lag_sec", "")),
                total_supply_estimate=parse_optional_decimal(row.get("total_supply_estimate", "")),
                mint_event_count=int(row.get("mint_event_count", "0") or 0),
                mint_total_amount=parse_optional_decimal(row.get("mint_total_amount", "")),
                mint_to_pair_amount=parse_optional_decimal(row.get("mint_to_pair_amount", "")),
                mint_to_pair_supply_ratio=parse_optional_decimal(row.get("mint_to_pair_supply_ratio", "")),
                mint_cum_to_pair_amount=parse_optional_decimal(row.get("mint_cum_to_pair_amount", "")),
                mint_last_to_pair_ts=parse_iso8601(row["mint_last_to_pair_ts"]) if row.get("mint_last_to_pair_ts", "").strip() else None,
                dump_swap_count=int(row.get("dump_swap_count", "0") or 0),
                dump_sell_volume=parse_optional_decimal(row.get("dump_sell_volume", "")),
                dump_sell_tot_supply_ratio=parse_optional_decimal(row.get("dump_sell_tot_supply_ratio", "")),
                dump_sell_to_reserve_max_ratio=parse_optional_decimal(row.get("dump_sell_to_reserve_max_ratio", "")),
                dump_sell_to_reserve_avg_ratio=parse_optional_decimal(row.get("dump_sell_to_reserve_avg_ratio", "")),
                dump_sell_abs_max=parse_optional_decimal(row.get("dump_sell_abs_max", "")),
                dump_quote_out_volume=parse_optional_decimal(row.get("dump_quote_out_volume", "")),
                dump_first_ts=parse_iso8601(row["dump_first_ts"]) if row.get("dump_first_ts", "").strip() else None,
                dump_last_ts=parse_iso8601(row["dump_last_ts"]) if row.get("dump_last_ts", "").strip() else None,
                dump_window_span_seconds=parse_optional_float(row.get("dump_window_span_seconds", "")),
                mint_to_dump_latency_sec=parse_optional_float(row.get("mint_to_dump_latency_sec", "")),
                dump_to_mint_ratio=parse_optional_decimal(row.get("dump_to_mint_ratio", "")),
            )
            features.setdefault(token_idx, []).append(window)
