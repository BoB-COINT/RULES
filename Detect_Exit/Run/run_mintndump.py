#!/usr/bin/env python3
"""CLI to evaluate STE0302 mint-and-dump rule against generated features."""
...
from rule_engine import evaluate, load_events, load_mintndump_features, load_rule


def main(argv: Optional[Sequence[str]] = None) -> None:
    parser = argparse.ArgumentParser(
        description="Apply the STE0302 mint-and-dump rule to feature windows"
    )
    parser.add_argument("--features", required=True, help="Path to mint-and-dump feature CSV")
    ...
    parser.add_argument(
        "--token-events",
        help="Optional token event CSV to enrich evidence if evaluator requests it",
    )
    ...
    features_by_token = load_mintndump_features(features_path)
