...
# Tunable parameters for slow-drain aggregation
...
def read_events(path: str) -> Dict[str, List[Event]]:
...
def finalize_window(
    ...
    burn_to_mint_ratio: Optional[float]
    if stats.mint_events == 0:
        burn_to_mint_ratio = float(stats.burn_events) if stats.burn_events else None
    else:
        burn_to_mint_ratio = stats.burn_events / stats.mint_events
    ...
    return {
        ...
        "burn_to_mint_ratio": float_to_str(burn_to_mint_ratio),
...
def main() -> None:
    ...
    parser.add_argument("--token-data", default="Token_onData.csv", help="Path to Token_onData CSV")
    parser.add_argument("--output", default="feature_slow.csv", help="Output CSV path")
    args = parser.parse_args()

    events_by_token = read_events(args.token_data)
    ...
    write_features(args.output, all_rows)
    print(f"Generated {len(all_rows)} windows to {args.output}")
